...
def main(name,savename=None):
    ...
    for yuzhi999 in [0,20,15,10,5]: #========0表示自由裁剪一次.
      try:
        ...
        if 0:
            ...
            corners = cv2.goodFeaturesToTrack(gray, 4, 0.0001, 50,useHarrisDetector=False)
            tmp10000=image.copy()
            for i in corners:
                tmp10000=cv2.circle(tmp10000,(int(i[0][0]),int(i[0][1])),0,(255,255,0),1)
            if debug:
                cv2.imwrite('dsafjdsaklfjalsdkjf.png',tmp10000)
            #=======corner方法绘制box
            aaaaa3=image.copy()
            aaaaa3=cv2.rectangle(aaaaa3,(round(corners[0][0][0]),round(corners[0][0][1])),(round(corners[3][0][0]),round(corners[3][0][1])),(0,0,255),0)
        ...
        if 1:
            ...
            # 边缘检测, Sobel算子大小为3
                ...
                lines = cv2.HoughLinesP(edges, 1, 0.01*np.pi / 180, round((gao+chang)/40), minLineLength=(gao+chang)/20, maxLineGap=(gao+chang)/20) #########0.01*pi是调优过的参数!!!!!!!!
                fffff=tmp.copy()
                ...
                if debug:
                 if 0:
                    cv2.imwrite('13方法2的画全部的线-1.png',fffff)
                #==========话lines
    ...
    #函数功能：找到图像里的点和轮廓之间的最短距离. 它返回的距离当点在轮廓外的时候是负值，当点在轮廓内是正值，如果在轮廓上是0。
                ...
                # 获取坐标 
                    x1, y1, x2, y2 = line[0]
                    cv2.line(fffff999, (x1, y1), (x2, y2), (0, 170, 170), thickness=1)
                if debug:
                 if 0:
                ...
                for aaa in [a1,a2]:
                    ...
                    if baocun:
                        ...
                        list1=[dex for dex,i in enumerate(a3) if abs(i-j)<yuzhi]
                        j=a3[baocun[1]]  
                        list2=[dex for dex,i in enumerate(a3) if abs(i-j)<yuzhi]
...
###########2023-10-09,10点45 刚才才发现, cv里面高度是距离图片左上角的高度差.不是我们数学上的针对左下角坐标原点的坐标.所以直接带入原始的点斜式方程求解出来的是错误的. 惊了.需要重新做坐标的归一化啊. 应该加一个负号即可,求解完再换算回来.
                def cross_point_dianxie(line1, line2):  # 计算交点函数====点斜型直线. +==========斜率最大是99999, 也就是垂直直线的斜率用这个数值表示.
                    #是否存在交点
                    point_is_exist=False
                    x=0
                    y=0
                ...
                for dex,i in enumerate(lines):
                    ...
                    a=min(math.tan(jiajiaobaocun[dex]/180*math.pi),99999) # 90度时候会溢出,所以上线设置10w即可.
                    lines_2.append(a)
                ...
                if len(zhixianfenzu)==4:
                    ...
                    zhixianchagndu=((zhixian999[2]-zhixian999[0])**2+(zhixian999[3]-zhixian999[1])**2)**0.5
                    ...
                    zhongdian=np.average(lines[xiamianzuizuo],axis=0,weights=wt1)
                    zhongdian=(zhongdian[0][0]+zhongdian[0][2])/2,(zhongdian[0][1]+zhongdian[0][3])/2,
                    zhongdian2=np.average(lines[chuizhizuoxia],axis=0,weights=wt2)
                    zhongdian2=(zhongdian2[0][0]+zhongdian2[0][2])/2,(zhongdian2[0][1]+zhongdian2[0][3])/2,
                    zuoxiajiao=cross_point_dianxie([zhongdian[0],zhongdian[1],jiaoduhuaxielv(np.average(np.array(jiajiaobaocun)[xiamianzuizuo],weights=wt1))],
                                        [zhongdian2[0],zhongdian2[1],jiaoduhuaxielv(np.average(np.array(jiajiaobaocun)[chuizhizuoxia],weights=wt2))]
                    ...
                    youxiajiao=cross_point_dianxie(
                        ...
                        [zhongdian[0],zhongdian[1],jiaoduhuaxielv(np.average(np.array(jiajiaobaocun)[xiamianzuizuo],weights=wt1))],
                                        [zhongdian2[0],zhongdian2[1],jiaoduhuaxielv(np.average(np.array(jiajiaobaocun)[chuizhizuoxia],weights=wt2))]
                ...
                # 获取坐标 
                    ...
                    fffff=cv2.line(fffff, (x1, y1), (x2, y2), (0, 170, 170), thickness=1)
                if debug:
        ...
        ########=============再加上取毛边的算法:
                ...
                if 1:
                    ...
                    for i8 in range(4):
                        ...
                        tmppoint=all3[i8]
                        bianchang=3 #=========2023-10-09,11点45超出变长的不讨论, 这样能保证缺角时候不会进行过度后处理.
                        ...
                        for i3 in candidate:
                            ...
                            for i4 in round2:
                                if (i4[1]<=fffff.shape[0] and i4[0]<=fffff.shape[1]):
                                    ...
                                    sedu.append(gray[i4[1],i4[0]]>100) #使用binary来判断是否存在根准确.
                                else:
                                    sedu.append(0)


                            all_sedu=sum(sedu)
                            if all_sedu>=1  and  sum(fffff[i3[1],i3[0]])>=0: # 加判断, 候选点本身也要有亮度!, 我们让他正好踩到图像的边.????????????????????????这个地方设置1,2,3????????看看实际效果吧, 总感觉2可能更好. 还是1好,测试了.
                                all3[i8]=i3
                                # print(1)
                ...
                for i in all3:
                    tmp3=cv2.circle(tmp3,(round(i[0]),round(i[1])),00,(255,255,255),0)
                if debug:
...
if __name__=='__main__':
    ...
    print(len(a))
    main('saving100v15ZZtrue/7878.bmp','output99999.png')
